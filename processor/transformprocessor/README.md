...
`convert_exponential_histogram_to_histogram([ExplicitBounds])`
...
The conversion assumes observations are log-uniformly distributed within each non-zero exponential bucket. The zero bucket is distributed linearly over its configured zero threshold. Randomized systematic rounding preserves the exact total count and keeps cumulative rounding error below one observation.
...
- The process of converting an ExponentialHistogram to an Explicit Histogram is not perfect and may result in a loss of precision. It is important to define an appropriate set of bucket boundaries and identify the best distribution approach for your data in order to minimize this loss. 
  ...
  For example, selecting boundaries that are too high or too low may result in histogram buckets that are too wide or too narrow, respectively.

- Positive, negative, and zero buckets are all converted. The output bucket counts always sum to the source data point count.

- The output contains one more bucket count than explicit bounds, including the required `+Inf` overflow bucket.
...
- `convert_exponential_histogram_to_histogram([0.0, 10.0, 100.0, 1000.0, 10000.0])`
