...
func TestAcquireSimpleNoWaiters(t *testing.T) {
	...
	bq := NewBoundedQueue(noopTelemetry, int64(maxLimitBytes), int64(maxLimitWaiters)).(*BoundedQueue)
...
func TestAcquireBoundedWithWaiters(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			bq := NewBoundedQueue(noopTelemetry, tt.maxLimitBytes, tt.maxLimitWaiters).(*BoundedQueue)
